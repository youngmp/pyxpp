...
void do_main_py(argc,argv)
char **argv;
int argc;
{
  /* python-specific main function */
  int ii;
  for (ii = 0; ii < argc; ii++)
    {
      printf("argv[%d] = %s\n", ii, argv[ii]);
  ...
  *tempNS = notAlreadySet;
  ...
  // initialize xpp vector
  init_alloc_info();

  // set internal options. ignore ~/.xpprc file for now.
  set_internopts_xpprc_and_comline();

  // set remaining numerical values that are not user-specified
  set_all_vals();


  init_alloc_info(); // init xppvec again?
  set_init_guess(); // self explanatory
  update_all_ffts(); // self explanatory
  
#ifdef AUTO 
  init_auto_win();
#endif

  printf("Version should be below here?\n");
  ...
  printf("Version should be above here?\n");

  do_meth(); // select method
  set_delay(); // self explanatory

  rhs=my_rhs; // seems like this is for convenience
  init_fit_info(); // for fitting functions
  strip_saveqn(); // stripes newlines and such. look for ascii table.

  create_plot_list(); // allocate list of plots?
  auto_load_dll(); // load dynamically linked lib from user
