...
void integrate_once_py(float* in1)
{
  FILE *fp;
  double *x;
  int i;
  ...
  // update x with input
  for(i=0;i<NODE+NMarkov;i++)x[i]=in1[i];
...
void batch_integrate_once()
{
  ...
  if(!batch_range || range.reset==0){
	...
	return;
      ...
      printf("write_mybrowser_data(fp)");
  ...
  printf("2nd write_mybrowser call ");
  write_mybrowser_data(fp);
